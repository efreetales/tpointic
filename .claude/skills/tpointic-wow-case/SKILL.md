...
9. **Galeria de impacto — pilha com parallax** (`gallery`) — em vez de blocos empilhados com espaço entre eles, cada bloco **cobre o anterior ao rolar** (efeito tipo Apple/Stripe). `gallery_intro` (opcional) entra como parágrafo logo abaixo do título — usar quando o conjunto de imagens não é autoexplicativo por conta própria (ex. Sulamérica: prints de emails/planilhas só fazem sentido com uma frase dizendo que aquilo foi o teste rápido, sem produto real, feito com Google Forms/Sheets/Apps Script). CSS puro, sem JS:
   ```tsx
